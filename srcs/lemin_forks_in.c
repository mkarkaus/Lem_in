...
int		first_check(t_graph *maze, int k, int path)
{
	int		i;
	int		row;
	...
	while (row < maze->paths)
	{
		...
		while (i + 1 < maze->max_level)
		{
			if (maze->route[row][i] == path)
				return (0);
			else if (maze->route[row][i] == k && maze->route[row][i + 1] != -1)
				return (1);
			i++;
		}
		row++;
...
void	pick_route(t_graph *maze, int *path, int infork, int index)
{
	int		i;
	int		k;
	int		m;
	...
	while (1)
	{
		...
		while (++m < maze->max_level)
		{
			if (maze->route[k][m] == infork)// valitsee kahdesta reitistä paremman
			{
				...
				// 	(k_out_forks == path_out_forks && (i > m || first_check(maze, k, *path))))
				if (path_out_forks == 0 || \
					(k_out_forks == path_out_forks && (i > m || first_check(maze, k, *path))))
				{
					del_route(maze, *path);
					del_allbutone(maze, maze->route[k][m - 1], infork, index);
				}
				else
				{
					del_route(maze, k);
					del_allbutone(maze, maze->route[*path][i - 1], infork, index);
					if (k < *path)
						(*path)--;
				}
				// *path -= ret;
